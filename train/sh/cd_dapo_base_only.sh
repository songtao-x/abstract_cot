...
# DAPO + base-only reward (no plan-aware delta terms).
#   Reward: base_reward_weight * base_reward
#     - think_delta_weight=0, reward_beta=0  → delta term zeroed out
#     - contrastive off
#   All other knobs identical to cd_dapo_answer_only.sh — clean baseline
#   for measuring what the answer-delta term actually buys you under DAPO.
...
#                                         zero-spread groups no longer zero the
#                                         advantage (the main gradient sink)
#
#   PHASE A tuning experiment (free, no extra compute): the two changes above
#   target the gradient-starvation diagnosed from dapo_abstract_base_only_n16
#   (32% zero-valid batches, base_reward flat ~0.48, entropy collapsing).
#   Output dir is tagged _phaseA so it does not overwrite the baseline run.

#SBATCH --output=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train/log/cd_dapo_base_only/%j.out
#SBATCH --error=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train/log/cd_dapo_base_only/%j.err

#SBATCH --job-name=dapo_base
#SBATCH --ntasks=1
#SBATCH --gres=gpu:4
#SBATCH --mem=300G
...
output_dir="$project_root/outputs/dapo_abstract_base_only_n16_phaseA"
num_processes=4
...
# --- reward composition (base-only: delta terms zeroed) ---
...
mkdir -p "$project_root/log/cd_dapo_base_only"

echo "[run_dapo] project_root=$project_root"
echo "[run_dapo] model=$model_name train=$train_file eval=$eval_file"
echo "[run_dapo] mode=base-only[phaseA]  loss=$loss_type  eps=$epsilon/$epsilon_high  q=$top_entropy_quantile  sr=$scale_rewards  G=$num_generations"

...
#SBATCH --output=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train/log/cd_grpo_contrastive/%j.out
#SBATCH --error=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train/log/cd_grpo_contrastive/%j.err

#SBATCH --job-name=grpo_contrast
#SBATCH --ntasks=1
#SBATCH --gres=gpu:4
#SBATCH --mem=300G
...
project_root=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train   # adjust
script="$project_root/grpo.py"

model_name="Qwen/Qwen3-4B"
train_file="$project_root/../data/cd4_train.jsonl"
eval_file="$project_root/../data/cd4_test.jsonl"
output_dir="$project_root/outputs/grpo_abstract_contrastive"
num_processes=4
...
mkdir -p "$project_root/log/cd_grpo_contrastive"
...
echo "[run_grpo] mode=contrastive CoT (weight=$contrastive_weight)"
...
# Bump NCCL collective timeout (default 600s). The contrastive path runs
# HF model.generate() under DeepSpeed ZeRO-3; each token-step triggers an
# all_gather of sharded params, which can exceed 600s and trip the watchdog.
# Crashed at trainer step ~10 in job 4781953 with this default.

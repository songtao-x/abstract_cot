...
# Reward = base_reward only.
#   think_delta_weight=0  (disable think-block logprob delta)
#   reward_beta=0         (disable answer logprob delta)
#   contrastive_cot off
# Use this run as the no-plan-bonus baseline.

#SBATCH --output=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train/log/cd_grpo_base_only/%j.out
#SBATCH --error=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train/log/cd_grpo_base_only/%j.err

#SBATCH --job-name=grpo_base
#SBATCH --ntasks=1
#SBATCH --gres=gpu:4
#SBATCH --mem=300G
...
output_dir="$project_root/outputs/grpo_abstract_base_only"
num_processes=4
...
mkdir -p "$project_root/log/cd_grpo_base_only"

echo "[run_grpo] project_root=$project_root"
echo "[run_grpo] model=$model_name train=$train_file eval=$eval_file"
echo "[run_grpo] mode=base-only (think_delta_weight=$think_delta_weight, reward_beta=$reward_beta)"

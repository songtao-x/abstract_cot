...
#   think_delta_weight=1   (only the think-block logprob delta is added)
#   reward_beta=0          (no answer-delta term)
#   contrastive_cot off
# Isolates the think-delta component of the combined reward.

#SBATCH --output=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train/log/cd_grpo_think_only/%j.out
#SBATCH --error=/home/songtaow/projects/aip-qchen/songtaow/reward_hack/abstract/train/log/cd_grpo_think_only/%j.err

#SBATCH --job-name=grpo_think
#SBATCH --ntasks=1
#SBATCH --gres=gpu:4
#SBATCH --mem=300G
...
output_dir="$project_root/outputs/grpo_abstract_think_only_n8"
num_processes=4
...
mkdir -p "$project_root/log/cd_grpo_think_only"

echo "[run_grpo] project_root=$project_root"
echo "[run_grpo] model=$model_name train=$train_file eval=$eval_file"
echo "[run_grpo] mode=think-only (think_delta_weight=$think_delta_weight, reward_beta=$reward_beta)"
